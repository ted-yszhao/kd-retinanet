...
\begin{tikzpicture}[
    ...
    teacher/.style={box, fill=blue!10, text width=34mm},
    student/.style={box, fill=green!10, text width=34mm},
    feat/.style={box, fill=orange!12, text width=32mm},
    proc/.style={box, fill=orange!8, text width=46mm},
    loss/.style={box, fill=red!10, text width=40mm},
    detloss/.style={box, fill=red!14, text width=40mm},
    note/.style={align=center, font=\small},
    flow/.style={->, rounded corners=3pt}
]

\node[box, fill=gray!8, text width=24mm] (input) at (0,8.8) {Input image\\batch $x$};

\node[teacher] (teacherbox) at (-4.1,6.4) {Frozen teacher\\RetinaNet-FPN\\ResNet-101/50};
\node[student] (studentbox) at (4.1,6.4) {Trainable student\\RetinaNet-FPN\\ResNet-18/50};
\coordinate (split) at (0,7.5);
\draw[flow] (input.south) -- (split);
\draw[flow] (split) -| (teacherbox.north);
\draw[flow] (split) -| (studentbox.north);

\node[feat] (tfeat) at (-4.1,4.0) {Selected teacher\\FPN levels\\$P_l^T,\; l\in\mathcal{P}$};
\node[feat] (sfeat) at (0.0,4.0) {Selected student\\FPN levels\\$P_l^S,\; l\in\mathcal{P}$};
\node[box, fill=orange!10, text width=40mm] (heads) at (4.6,4.0) {Student RetinaNet heads\\classification + box regression};
\draw[flow] (teacherbox.south) -- (tfeat.north);
\coordinate (studentsplit) at ($(studentbox.south)+(0,-0.5)$);
\draw[flow] (studentbox.south) -- (studentsplit);
\draw[flow] (studentsplit) -| (sfeat.north);
\draw[flow] (studentsplit) -- (heads.north);

\node[proc] (perlevel) at (-2.0,1.4) {Per selected level $l$:\\[0.8mm]align spatial size if needed\\normalize feature channels\\average to response maps\\compute windowed SSIM to obtain $\bar{\mathrm{SSIM}}_l$};
\draw[flow] (tfeat.south) -- ++(0,-0.4) -| ([xshift=-12mm]perlevel.north);
\draw[flow] (sfeat.south) -- ++(0,-0.4) -| ([xshift=12mm]perlevel.north);

\node[loss] (levelterm) at (-2.0,-1.2) {Level contribution\\$\alpha_l\!\left(1-\bar{\mathrm{SSIM}}_l\right)$};
\node[note] (repeatnote) at (-2.0,-2.4) {Repeat for all selected levels $l \in \mathcal{P}$};
\node[loss, text width=46mm] (kdterm) at (-2.0,-4.1) {Hierarchical KD term\\$\mathcal{L}_{kd}=\sum_{l\in\mathcal{P}}\alpha_l\left(1-\bar{\mathrm{SSIM}}_l\right)$};
\draw[flow] (perlevel.south) -- (levelterm.north);
\draw[flow] (levelterm.south) -- (repeatnote.north);
\draw[flow] (repeatnote.south) -- (kdterm.north);

\node[detloss] (det) at (4.6,1.4) {Detection loss\\$\mathcal{L}_{det}=\mathcal{L}_{cls}+\mathcal{L}_{box}$};
\draw[flow] (heads.south) -- (det.north);

\node[loss, text width=46mm] (total) at (1.3,-6.8) {Final student objective\\$\mathcal{L}=\mathcal{L}_{det}+\lambda \mathcal{L}_{kd}$};
\draw[flow] (kdterm.south) -- ++(0,-0.8) -| ([xshift=-2mm]total.north);
\draw[flow] (det.south) -- ++(0,-0.9) -| ([xshift=2mm]total.north);

\node[note, text=red!75!black] at (1.3,-8.5) {Backpropagation updates the student only; the teacher remains frozen.};
