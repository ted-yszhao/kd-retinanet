...
\hline
\texttt{vanilla\_retinanet\_resnet50} & none & ResNet-50 & no & Baseline mid-capacity detector \\
\hline
...
\caption{Experiment configurations recovered from the repository \texttt{json} files and saved run records. All runs use AdamW, batch size $6$, $10$ epochs, per-epoch subset sampling with ratio $0.3$, and COCO evaluation every epoch.}
...
The saved checkpoints further illustrate an important property of the proposed framework: in a distilled model, only the student is retained for deployment. The training code explicitly saves the state dictionary of the trainable branch rather than serializing the frozen teacher together with it. Consequently, both the baseline \texttt{ResNet-18} run and the two distilled \texttt{ResNet-18} runs produce final checkpoints of approximately $81$ MB, while both the vanilla and distilled \texttt{ResNet-50} student checkpoints are about $131$ MB. This is not a trivial engineering detail. It demonstrates that the proposed method preserves the main practical promise of knowledge distillation: the teacher increases training-time supervision but does not increase inference-time model size.
...
This resource profile clarifies why the \texttt{ResNet-18} setting is the most meaningful test case in the present study. Distilling from \texttt{ResNet-101} into \texttt{ResNet-50} may still be useful, but the deployment savings are relatively modest compared with the jump from \texttt{ResNet-101} to \texttt{ResNet-18}. If structural distillation can improve the smallest student without changing its checkpoint size, then the method addresses the efficiency objective directly rather than only shifting accuracy within already large detectors.
...
\hline
Run & mAP & $\Delta$ mAP & AP$_{50}$ & AP$_{75}$ & AP$_S$ & AP$_M$ & AP$_L$ \\
\hline
Vanilla ResNet-18 & 0.1515 & baseline & 0.2544 & 0.1569 & 0.0752 & 0.1631 & 0.1905 \\
\hline
ResNet-50 $\rightarrow$ ResNet-18 KD & 0.0000 & $-0.1515$ & 0.0000 & 0.0000 & 0.0000 & 0.0000 & 0.0000 \\
\hline
\textbf{ResNet-101 $\rightarrow$ ResNet-18 KD (proposed compact model)} & \textbf{0.1636} & \textbf{+0.0121} & \textbf{0.2741} & \textbf{0.1701} & \textbf{0.0809} & \textbf{0.1759} & \textbf{0.2157} \\
\hline
Vanilla ResNet-50 & 0.1830 & baseline & 0.3011 & 0.1917 & 0.0997 & 0.1968 & 0.2278 \\
\hline
\textbf{ResNet-101 $\rightarrow$ ResNet-50 KD} & \textbf{0.1954} & \textbf{+0.0124} & \textbf{0.3229} & \textbf{0.2038} & \textbf{0.1105} & \textbf{0.2104} & \textbf{0.2437} \\
\hline
\end{tabular}
}
\caption{Final COCO validation metrics extracted from the recorded experiment outputs in the repository. All scores are for the student branch evaluated on $5000$ validation images. Both successful \texttt{ResNet-101}-teacher runs improve over their matching student-only baselines, while the proposed compact model, \texttt{ResNet-101} $\rightarrow$ \texttt{ResNet-18} KD, remains the most important efficiency-oriented result because it improves the smallest deployment target without increasing its student checkpoint size.}
...
The new vanilla \texttt{ResNet-50} baseline makes the larger-student interpretation much stronger than before. Under the same budget-constrained schedule, the student-only \texttt{ResNet-50} RetinaNet reaches $0.1830$ mAP, while the \texttt{ResNet-101} $\rightarrow$ \texttt{ResNet-50} distilled run reaches $0.1954$. This corresponds to an absolute gain of $0.0124$ mAP, or roughly $6.8\%$ relative improvement over the vanilla \texttt{ResNet-50} score. The gains are again broad rather than isolated: AP$_{50}$ rises by $0.0218$, AP$_{75}$ by $0.0121$, AP$_S$ by $0.0108$, AP$_M$ by $0.0136$, and AP$_L$ by $0.0159$. This means the positive effect of the proposed structural KD objective is not limited to only the smallest student. The framework also appears capable of improving a mid-capacity detector while preserving the same deployed \texttt{ResNet-50} checkpoint size.
...
To make the comparison between the compact baseline and the successful distilled compact student more explicit, it is useful to summarize the epoch-wise mAP trajectory directly in the discussion. The vanilla \texttt{ResNet-18} student rises from $0.0053$ mAP at epoch $1$ to $0.1515$ at epoch $10$, while the distilled \texttt{ResNet-101} $\rightarrow$ \texttt{ResNet-18} run moves from $0.0075$ to $0.1636$ over the same schedule. The distilled student is already ahead at epoch $1$, reaches $0.0947$ by epoch $5$ compared with $0.0664$ for the baseline, and despite a temporary drop at epoch $8$ it still recovers to finish above the student-only model. Several features of the training dynamics are worth noting. First, both runs begin near zero, which is expected under dense detector training on COCO. Second, the distilled student is ahead of the baseline at the first epoch and preserves that advantage through most of training. Third, the margin is not perfectly monotonic: the two runs come quite close at epochs $3$ and $4$, the distilled student opens a clearer lead at epochs $5$ and $7$, and then briefly dips at epoch $8$ before recovering strongly in the last two epochs.

The \texttt{ResNet-50} pair shows a related but not identical pattern. The vanilla \texttt{ResNet-50} student moves from $0.0050$ mAP at epoch $1$ to $0.1830$ at epoch $10$, while the distilled \texttt{ResNet-101} $\rightarrow$ \texttt{ResNet-50} run rises from $0.0109$ to $0.1954$. The KD run is ahead immediately, opens a particularly large early lead by epoch $4$ ($0.1024$ versus $0.0566$), dips slightly below the vanilla run at epoch $5$, and then regains the advantage for the remainder of training. This strengthens the interpretation that structural distillation affects optimization trajectory rather than merely adding a small final-epoch boost. In both student sizes, the teacher-guided run can fluctuate locally while still converging to a better final detector.
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{../images/kd_loss_curves.pdf}
\caption{Epoch-averaged training losses for the two successful \texttt{ResNet-101}-teacher runs, computed from the recorded TensorBoard scalars \texttt{train/loss\_total} and \texttt{train/loss\_kd}. Each run is split into a broad-scale total-loss panel and a zoomed KD-loss panel so that the KD descent remains visually readable. In both cases the total student objective decreases steadily while the KD term also trends downward across epochs, indicating that structural supervision acts as a persistent auxiliary constraint rather than dominating optimization. The \texttt{ResNet-50} student also reaches a slightly lower final total loss than the compact \texttt{ResNet-18} student, which is consistent with the larger student's greater capacity to satisfy the detection and distillation objectives simultaneously.}
\label{fig:kd-loss-curves}
\end{figure}

Figure~\ref{fig:kd-loss-curves} adds a complementary view of the same optimization process by focusing directly on the logged loss terms. For the compact \texttt{ResNet-101} $\rightarrow$ \texttt{ResNet-18} run, the epoch-averaged total loss falls from approximately $1.61$ in the first epoch to $0.88$ by epoch $10$, while the KD term decreases more gently from about $0.394$ to $0.331$. The \texttt{ResNet-101} $\rightarrow$ \texttt{ResNet-50} run follows a very similar pattern but finishes slightly lower, with total loss reaching about $0.83$ and KD loss about $0.309$ at the final epoch. This is a useful diagnostic result because it shows that the KD branch remains active throughout training instead of collapsing immediately to zero, yet it also stays well below the total objective. In practical terms, the student is learning under sustained teacher guidance without the auxiliary loss overwhelming the detector's own task loss.
...
Third, the distillation design itself is only partially explored. The current implementation uses a single selected feature key, a single SSIM window size, and a fixed global KD weight. These decisions were appropriate for an initial controlled experiment, but they limit how confidently one can generalize from the present results to the broader space of structural detector distillation methods. Fourth, although the addition of a vanilla \texttt{ResNet-50} baseline strengthens the comparison set substantially, the experimental matrix is still incomplete. There are still no repeated seeds, no full ablation over teacher strength, and no alternative feature-loss baselines such as $\ell_1$ or $\ell_2$ matching under the same schedule. Fifth, the saved records for the \texttt{ResNet-101} $\rightarrow$ \texttt{ResNet-50} configuration span multiple output files, which suggests the run may have been resumed or repeated during development. The final reported metric is still usable, but the run history is less clean than that of the other experiments.

Despite these limitations, the experiments are still valuable. They are directly tied to the repository artefacts, they compare meaningful teacher-student settings, and they answer one of the most important practical questions in a non-trivial way: under a constrained training budget, can structural distillation improve a compact RetinaNet student without changing deployment cost? The answer provided by the \texttt{ResNet-101} $\rightarrow$ \texttt{ResNet-18} run is yes, at least for the implemented configuration and measured metrics.

\section{Summary}

The empirical evidence in this chapter turns the work from a purely methodological proposal into a supported experimental study. A compact \texttt{ResNet-18} RetinaNet baseline reaches $0.1515$ mAP under the current budget-constrained schedule, while the proposed structural distillation method with a frozen \texttt{ResNet-101} teacher improves the same student to $0.1636$ mAP and raises all other reported COCO metrics. The new vanilla \texttt{ResNet-50} baseline also shows that the larger-student \texttt{ResNet-101} $\rightarrow$ \texttt{ResNet-50} run is a genuine KD gain rather than only a high-capacity reference, improving from $0.1830$ to $0.1954$ mAP under the same schedule. At the same time, the failure of the \texttt{ResNet-50} $\rightarrow$ \texttt{ResNet-18} run reveals that stability cannot be taken for granted and that teacher-student pairing and KD hyperparameters matter. These findings motivate a balanced conclusion: the proposed post-FPN SSIM-based distillation strategy is promising for efficient one-stage object detection, but its full potential depends on more systematic ablation and stabilization in future work.
